    ...
    public class MaskedTextProvider //: ICloneable
    {
        ...
        public MaskedTextProvider(string mask, CultureInfo culture, bool allowPromptAsInput, char promptChar, char passwordChar, bool restrictToAscii)
        {
            if (string.IsNullOrEmpty(mask))
            {
                ...
                throw new ArgumentException("MaskedTextProvider Mask Null Or Empty", "mask");
            }
            ...
            foreach (char c in mask)
            {
                if (!IsPrintableChar(c))
                {
                    ...
                    throw new ArgumentException("MaskedTextProvider Mask Invalid Char");
                }
            }
            ...
            for (int maskPos = 0; maskPos < this.mask.Length; maskPos++)
            {
                ...
                if (!escapedChar)   // if false treat the char as literal.
                {
                    switch (ch)
                    {
                        ...
                        case ':':   // time separator.
                            {
                                var tmpDate = DateTime.Now.ToString("t");
                                foreach (var timeChar in tmpDate)
                                {
                                    if (char.IsDigit(timeChar))
                                        continue;

                                    locSymbol = timeChar.ToString();
                                    break;
                                }
                        ...
                        case '/':   // date separator.
                            {
                                var tmpDate = DateTime.Now.ToString("d");
                                foreach (var dateChar in tmpDate)
                                {
                                    if (char.IsDigit(dateChar))
                                        continue;

                                    locSymbol = dateChar.ToString();
                                    break;
                                }
        ...
        public char PasswordChar
        {
            ...
            set
            {
                if (value == this.promptChar)
                {
                    ...
                    throw new InvalidOperationException("Prompt and password chars must be different.");
                }
                ...
                if (!IsValidPasswordChar(value) && (value != nullPasswordChar))
                {
                    ...
                    throw new ArgumentException("Same message as in SR.MaskedTextBoxInvalidCharError.");
                }
        ...
        private bool TestChar(char input, int position, out MaskedTextResultHint resultHint)
        {
            ...
            switch (this.mask[charDex.MaskPosition])
            {
                ...
                default:
                    Debug.WriteLine("Invalid mask language character.");
                    break;
            }
